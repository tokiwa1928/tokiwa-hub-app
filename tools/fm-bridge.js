...
(function (global) {
  ...
  var 引数名 = {
    ...
    '画面_流用新規':       ['recordId', '種別', '同じ案件'],   // RYUYO-1／STAGE-1
    '画面_削除':           ['recordId', 'modId'],
    '画面_外注':           ['伝票番号'],
    '画面_外注保存':       ['recordId', 'modId', '行'],
    '画面_外注作成':       ['伝票番号', '行'],
    '画面_全項目':         ['recordId'],
    '画面_履歴':           ['recordId', '伝票番号'],   // SEIZO-4: 出力後に直ったかの判定
    '画面_案件にまとめる': ['recordIds', '案件ID', '確認'],   // GROUP-1: 本体の案件管理から（伝票番号でも可）
    '指示書_記録':         ['記録'],   // SEIZO-7: 製造指示書の出力記録（QR 確認用）
    '手配_読む':           ['伝票番号'],          // TEHAI-1: 手配（仕入／在庫／外注先手配・部分外注・発送・箱数）
    '手配_書く':           ['伝票番号', '手配'],
    '手配_一覧':           [],                    // MITEI-1: 未確定・未発注の一覧
    '単価_集計':           ['年数'],              // TANKA-1
    'マスタ_書く':         ['layout', 'recordId', '項目', '前'],   // MASTER-1
    'マスタ_作る':         ['layout', '項目'],
    'Hubマスタ_一覧':      ['名'],
    'Hubマスタ_書く':      ['名', '行'],
    'Hubマスタ_まとめて書く': ['名', '行'],
    'Hubマスタ_消す':      ['名', 'id'],
    '画面項目_退避':       ['id', 'label', '状態'],   // HIDE-1
    '手配_発注済':         ['伝票番号', '種', 'n', 'done'],   // SHIIRE-1
    '画面項目_使った':     ['rows'],
    '手配_写す':           ['元', '先'],          // SPEC-1
    '仕様書_置く':         ['番号', '名', 'mime', 'base64'],
    '仕様書_一覧':         ['番号'],
    '予定_一覧':           ['開始', '終了'],
    '予定_作る':           ['日付', 'タイトル', '時刻'],
