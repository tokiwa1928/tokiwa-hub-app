...
(function () {
  ...
  function 外注合計を出す() {
    ...
    Array.prototype.forEach.call($('gc-rows').querySelectorAll('tr'), function (tr) {
      ...
      var s = Math.round(q * p); tr.querySelector('.gc-sum').textContent = (q || p) ? s.toLocaleString() : ''; 総 += s;
    });
  ...
  帯に付ける('新規作成', function () {
    var n = document.querySelector('#fmbar button.new[data-kind="受注"]') || document.querySelector('#fmbar button.new');
    if (n) n.click();
  }, 'FileMaker に新しい受注を起こします（黒帯の「受注」と同じ）');
