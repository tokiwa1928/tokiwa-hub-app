...
(function () {
  ...
  function 製造記録を読む() {
    return new Promise(function (res) {
      ...
      var m = {};
      ((db && db.sagyoPrints) || []).forEach(function (r) { if (!r || !r.fmNo) return; var k = String(r.fmNo); if (!m[k] || String(r.at || '') > String(m[k].at || '')) m[k] = { n: r.n, at: r.at, by: r.by, pc: r.pc, comment: r.comment }; });
      try { (JSON.parse(localStorage.getItem('fm_sagyo_pending') || '[]')).forEach(function (r) { if (!r || !r.fmNo) return; var k = String(r.fmNo); if (!m[k] || String(r.at || '') > String(m[k].at || '')) m[k] = { n: r.n, at: r.at, by: r.by, pc: r.pc, comment: r.comment }; }); } catch (e) {}
      製造記録 = m; return m;
    });
  }
  ...
  function 一覧を出す(r) {
    ...
    var kkh = 権限();
    列.forEach(function (k) { if (k.c === '合計金額' && kkh.partner) return; var hd = k.h; if (k.tokiwaPrice && kkh.partner) hd = '仕入(トキワ売価)'; if (k.senpo && kkh.partner) hd = '自社売価'; h += '<th>' + esc(hd) + '</th>'; });   // GRP-2: 関連会社にはトキワの原価を出さない
    h += '</tr></thead><tbody>';
    ...
    function 行の札(row) {
      ...
      列.forEach(function (k) {
        var v = row[k.c];
        if (k.c === '合計金額' && 権限().partner) return;   // GRP-2
