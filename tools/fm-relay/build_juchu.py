...
(function () {
  ...
  function ガイドを描く(){
    ...
    var 表 = 原価表(); var ctx = '品種:' + String(f['品種'] || '') + ' ' + String(f['製本加工'] || '') + ' ' + String(f['製品名'] || '') + ' ' + String(($('f-prod') || {}).value || '') + ' ' + String(($('f-bind') || {}).value || '');
    ...
    var 項目 = 項目定義.map(function (d) {
      var m = 原価行(d.名);
      var 固定 = (m && !空か(m.固定)) ? 数値(m.固定) : d.固定既定;
      var 千 = (m && !空か(m.千単価)) ? 数値(m.千単価) : (q0 ? d.比例 / q0 * 1000 : 0);
      var 最低原価 = (m && !空か(m.最低原価)) ? 数値(m.最低原価) : 0;
      var 率 = (m && !空か(m.掛け率)) ? 数値(m.掛け率) : 0;
      var 最低売価 = (m && !空か(m.最低売価)) ? 数値(m.最低売価) : 0;
      return { 名: d.名, 固定: 固定, 千: 千, 最低原価: 最低原価, 率: 率, 最低売価: 最低売価, 表: !!m, 条件: m ? String(m.条件 || '') : '' };
    });
    ...
    var 表の項目 = 項目.filter(function (x) { return x.表; }).map(function (x) { return x.名 + (x.条件 ? '（' + x.条件 + '）' : ''); });
    ...
    var rows = 階段.map(function (n) {
      ...
      項目.forEach(function (x) {
        ...
        var p = null; if (x.率 > 0) { p = Math.max(x.最低売価, c * x.率); 売価表 += p; } else 原価pool += c;
