...
(function () {
  ...
  function 円(v){ return Math.round(v).toLocaleString(); }
  ...
  function ガイドを描く(){
    ...
    var h = '<div><span class="k">種:</span> <b>' + esc(ガイド種.番号) + '</b> ' + esc(f['製品名'] || '') + '　起票 ' + esc(日付(f['起票日'])) + '（' + 経過 + ' 年前 → 目安 +' + u + '%）'
      + '　数量 ' + 円(q0) + '　単価 ' + (単価0 ? 単価0.toFixed(1) : '—') + '　売価 ¥' + 円(売価0) + '　原価 ¥' + 円(原価0) + '　原価率 ' + (率0 * 100).toFixed(1) + '%'
      ...
      + '<div style="color:#64748b;font-size:11px">内訳（種）: 版 ' + 円(版) + '／用紙 ' + 円(用紙) + '／印刷 ' + 円(印刷) + '／加工 ' + 円(加工) + '／外注 ' + 円(外注0) + '／梱包・配送 ' + 円(梱包 + 配送) + '　※版代は固定、印刷代は ' + Math.round(pf * 100) + '% 固定、ほかは数量比例。段をクリックで数量と単価をフォームへ</div>'
      + '<table><thead><tr><th>数量</th><th>原価</th><th>売価</th><th>単価</th><th>粗利率</th><th>前回単価比</th></tr></thead><tbody>'
      + rows.map(function (r) { return '<tr class="pick' + (r.n === qNow ? ' now' : '') + '" data-n="' + r.n + '" data-u="' + r.単価 + '"><td>' + 円(r.n) + (r.n === qNow ? ' ◀ 今回' : '') + '</td><td>¥' + 円(r.原価) + '</td><td>¥' + 円(r.売価) + '</td><td>' + r.単価.toFixed(1) + '</td><td>' + r.粗利.toFixed(1) + '%</td><td>' + (r.前年比 >= 0 ? '+' : '') + r.前年比.toFixed(1) + '%</td></tr>'; }).join('')
      + '</tbody></table>';
