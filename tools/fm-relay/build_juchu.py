...
<style>
  ...
  /* HIDE-1: 退避した項目と退避モード */
  .fm-taihi{ display:none !important; }
  body.taihi-mode input:hover, body.taihi-mode select:hover, body.taihi-mode textarea:hover, body.taihi-mode button:hover, body.taihi-mode .fb:hover{ outline:2px dashed #b91c1c !important; outline-offset:1px; cursor:crosshair !important; }
  body.taihi-mode #fm-shelf *{ outline:none !important; cursor:default !important; }
  #fm-shelf{ display:none; position:fixed; right:12px; top:60px; width:min(560px, 96vw); max-height:calc(100vh - 80px); overflow:auto; background:#fff; border:2px solid #b91c1c; box-shadow:0 8px 30px rgba(0,0,0,.25); z-index:9000; font-size:12px; padding:8px 12px; }
  #fm-shelf table{ border-collapse:collapse; width:100%; margin-top:4px; }
  #fm-shelf th{ background:#fee2e2; text-align:left; padding:3px 6px; font-weight:600; white-space:nowrap; }
  #fm-shelf td{ padding:3px 6px; border-bottom:1px solid #f1f5f9; vertical-align:top; }
  #fm-shelf button{ font:inherit; font-size:11px; padding:1px 8px; }
  #fm-shelf .old{ color:#b91c1c; font-weight:700; }
...
<div id="fmguide">
  <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap">
    ...
    <span style="margin-left:auto;color:#166534">目安: 昨年 <input class="n" id="pg-u1" value="5">% ／ 一昨年 <input class="n" id="pg-u2" value="10">% ／ それ以前 <input class="n" id="pg-u3" value="20">%　印刷代の固定分 <input class="n" id="pg-pf" value="30">%　最低受注金額 ¥<input class="n" id="pg-min" value="" style="width:56px" title="合計売価がこれを下回るときはこの金額にします。原価テーブルの「最低受注金額」の行があればそれが既定"> <button id="pg-detail" type="button" style="font-size:11px;padding:1px 6px">内訳を出す</button> <a href="master.html?t=原価テーブル" target="_blank" style="color:#166534">原価テーブル</a></span>
  </div>
...
<div id="fmkin">
  ...
  <div id="kn-yosan" style="display:none;margin-top:4px;padding:5px 8px;border:1px solid #93c5fd;background:#eff6ff;border-radius:3px"></div>
...
(function () {
  ...
  // COVER-1: 表紙行（p0/k0/w0）は FileMaker の「…0」列（紙質0・色数0・インキ表0・版下0・横ミシン0 …）。1 行目の対応から機械的に作る
  ['p', 'k', 'w'].forEach(function (t) {
    Object.keys(FMUSE).forEach(function (id) {
      if (id.indexOf(t + '1-') !== 0) return; var col = FMUSE[id].c; if (!col || col.indexOf('FM') === 0 || !/1$/.test(col)) return;
      var id0 = id.replace(t + '1-', t + '0-'), col0 = col.replace(/1$/, '0');
      TO_FM[id0] = col0; (FROM_FM[col0] = FROM_FM[col0] || []).push(id0);
    });
  });
  var 表紙の項目 = Object.keys(TO_FM).filter(function (id) { return /^[pkw]0-/.test(id); });
  ...
  function 流し込む(rec) {
    ...
    try { calc(); } catch (e) {}
    try { onInk(); } catch (e) {}
    try { 表紙を決める(null); } catch (e) {}
    try { 印を入れる(rec.fields); } catch (e) {}
    try { applyRowVis(); } catch (e) {}
    try { refreshNote(); } catch (e) {}
  }
  // STAMP-1（本多さん 9/26）: 右上の 作成者・作成日・作成時刻／修正者・修正日・修正時刻 は FileMaker の値。読み込んだ時刻ではない。
  // 項目名はレイアウトによって違うので候補を順に見る。タイムスタンプしか無ければ日付と時刻に分ける。無い項目は「—」
  function 印を入れる(f) {
    f = f || {}; var pick = function (names) { for (var i = 0; i < names.length; i++) { var v = f[names[i]]; if (v !== undefined && v !== null && String(v).trim() !== '') return String(v).trim(); } return ''; };
    var 短 = function (d) { var m = /^(\d{1,2})\/(\d{1,2})\/(\d{4})/.exec(d || ''); return m ? m[3].slice(2) + '/' + ('0' + m[1]).slice(-2) + '/' + ('0' + m[2]).slice(-2) : (d || ''); };
    var 分ける = function (ts) { var m = /^(\d{1,2}\/\d{1,2}\/\d{4})\s+(\d{1,2}):(\d{2})/.exec(ts || ''); return m ? { d: 短(m[1]), t: ('0' + m[2]).slice(-2) + ':' + m[3] } : null; };
    var c = 分ける(pick(['作成タイムスタンプ', '作成情報タイムスタンプ', '作成日時'])), m = 分ける(pick(['修正タイムスタンプ', '修正情報タイムスタンプ', '修正日時']));
    var put = function (id, v) { var e = $(id); if (e) e.textContent = v || '—'; };
    put('st-cu', pick(['作成者', '作成者名', '作成アカウント', '作成アカウント名'])); put('st-mu', pick(['修正者', '修正者名', '修正アカウント', '修正アカウント名']));
    put('st-cd', 短(pick(['作成日', '作成日付'])) || (c && c.d) || 短(pick(['起票日']))); put('st-md', 短(pick(['修正日', '修正日付'])) || (m && m.d));
    put('st-ct', pick(['作成時刻', '作成時間']) || (c && c.t)); put('st-mt', pick(['修正時刻', '修正時間']) || (m && m.t));
    var box = document.querySelector('.stampbox'); if (box) { var ks = Object.keys(f).filter(function (k) { return /作成|修正/.test(k); }); box.title = 'FileMaker の項目: ' + (ks.length ? ks.join('、') : '作成・修正の項目がレイアウトにありません'); }
  }
  window.FM印 = 印を入れる;
  // COVER-1: 表紙の有無。手配に覚えた値 > 表紙行に何か入っている > 既存データは「あり」。名刺・封筒などの簡単な品目だけ「なし」
  function 表紙を決める(o) {
    var el = $('f-cover'); if (!el) return;
    var 記憶 = (o && o.表紙) || '';
    var 入っている = 表紙の項目.some(function (id) { var x = $(id); return !!(x && String(x.value || '').trim()); });
    var 簡単 = (typeof 簡単か === 'function') ? 簡単か() : false;
    el.value = 記憶 ? 記憶 : (入っている ? 'あり' : (簡単 ? 'なし' : 'あり'));
    if (el.value !== 'あり' && el.value !== 'なし') el.value = 'あり';
    try { applyRowVis(); } catch (e) {}
  }
  ...
  function ガイドを描く(){
    ...
    // KAKAKU-2: 項目ごとの原価テーブル（Hub マスタ）。行が無い項目は今までどおり（種から比例・原価率を保つ）
    // KAKAKU-3: 台数 ＝ 各パーツ（表紙も）の 表の色数＋裏の色数 の合計。フォームの値を優先し、無ければ種の伝票（色数n／色MAXn）。色数の無いパーツは 1
    var 台数 = 0, 最大色 = 0, 部 = 0, 色あり = false, カラー部 = 0;   // KAKAKU-4: カラー（4 色以上）のパーツはオンデマンド・外注なので版を数えない
    for (var pi = 0; pi <= 9; pi++) {
      var ki1 = 数値(($('k' + pi + '-iro1') || {}).value) || (pi ? 数値(f['色数' + pi]) : 0), ki2 = 数値(($('k' + pi + '-iro2') || {}).value) || (pi ? 数値(f['色MAX' + pi]) : 0);
      var ある = ki1 || ki2 || String(($('p' + pi + '-kind') || {}).value || '').trim() || (pi ? String(f['紙質' + pi] || '').trim() : '');
      if (pi === 0 && String(($('f-cover') || {}).value || '') !== 'あり') ある = ki1 || ki2;
      if (!ある) continue; 部++; 最大色 = Math.max(最大色, ki1, ki2); if (ki1 || ki2) 色あり = true;
      if (Math.max(ki1, ki2) >= 4) { カラー部++; continue; }   // カラー＝オンデマンド・外注。版は 0 台
      台数 += (ki1 + ki2);
    }
    if (!色あり) 台数 = Math.max(1, 部);   // どのパーツにも色数が無い（古い伝票）ときはパーツ数
    var 表 = 原価表(); var ctx = '品種:' + String(f['品種'] || '') + ' ' + String(f['製本加工'] || '') + ' ' + String(f['製品名'] || '') + ' ' + String(($('f-prod') || {}).value || '') + ' ' + String(($('f-bind') || {}).value || '') + ' ' + String(f['備考'] || '') + ' ' + String(($('f-memo') || {}).value || '')
      + ' ' + (最大色 >= 4 ? 'カラー 4色' : 最大色 >= 2 ? '2色 単色系' : '単色 モノクロ 1色') + ' 台数:' + 台数 + ' 色数:' + 最大色;
    function 原価行(項目) { var hit = null; 表.forEach(function (r) { if (String(r.項目 || '').trim() !== 項目 || r.消) return; var c = String(r.条件 || '').trim(); if (!c) { if (!hit) hit = r; return; } var ok = c.split(/[,、\s]+/).filter(Boolean).some(function (w) { return ctx.indexOf(w) >= 0; }); if (ok && (!hit || !String(hit.条件 || '').trim())) hit = r; }); return hit; }
    function 空か(v) { return v === undefined || v === null || String(v).trim() === ''; }
    var 項目定義 = [
      { 名: '版代',   種: 版,   固定既定: 版,        比例: 0 },
      { 名: '用紙',   種: 用紙, 固定既定: 0,         比例: 用紙 * (1 + 用紙差 / 100) },
      { 名: '印刷代', 種: 印刷, 固定既定: 印刷 * pf, 比例: 印刷 * (1 - pf) },
      { 名: '加工賃', 種: 加工, 固定既定: 0,         比例: 加工 },
      { 名: '外注',   種: 外注0, 固定既定: 0,        比例: 外注0 },
      { 名: '梱包代', 種: 梱包, 固定既定: 0,         比例: 梱包 },
      { 名: '配送代', 種: 配送, 固定既定: 0,         比例: 配送 },
      { 名: '人件費', 種: 人件, 固定既定: 0,         比例: 人件 }
    ];
    var 項目 = 項目定義.map(function (d) {
      var m = 原価行(d.名);
      var 倍 = (m && /^(有|1|true|○|はい)$/i.test(String(m.台数掛け || '').trim())) ? 台数 : 1;   // KAKAKU-3: 台数掛け
      var 固定 = (m && !空か(m.固定)) ? 数値(m.固定) * 倍 : d.固定既定;
      var 千 = (m && !空か(m.千単価)) ? 数値(m.千単価) : (q0 ? d.比例 / q0 * 1000 : 0);
      var 最低原価 = (m && !空か(m.最低原価)) ? 数値(m.最低原価) * 倍 : 0;
      var 率 = (m && !空か(m.掛け率)) ? 数値(m.掛け率) : 0;
      var 最低売価 = (m && !空か(m.最低売価)) ? 数値(m.最低売価) * 倍 : 0;
      return { 名: d.名, 固定: 固定, 千: 千, 最低原価: 最低原価, 率: 率, 最低売価: 最低売価, 表: !!m, 条件: m ? String(m.条件 || '') : '', 倍: 倍 };
    });
    ...
    var rows = 階段.map(function (n) {
      // KAKAKU-2: 項目ごとに 原価(n) = max(最低原価, 固定 + 千単価×n/1000)。掛け率のある項目はその売価、無い項目はまとめて種の原価率＋上げ
      var 内訳 = [], 原価n = 0, 売価表 = 0, 原価pool = 0;
      項目.forEach(function (x) {
        var c = Math.max(x.最低原価, x.固定 + x.千 * n / 1000); if (!(c > 0)) c = 0;
        var p = null; var 率x = (率0 ? 1 / 率0 : 1.5) * (1 + 上げ / 100);   // 昨年の原価率＋上げ
        if (x.率 > 0) { p = Math.max(x.最低売価, c * x.率); 売価表 += p; }                                  // 掛け率あり: 原価×掛け率
        else if (x.表) { p = Math.max(x.最低売価, x.固定 + Math.max(0, c - x.固定) * 率x); 売価表 += p; }   // KAKAKU-3: 掛け率なし: 固定はそのまま売価（固定＝最低売価）、数量分だけ昨年の率
        else 原価pool += c;
        原価n += c; 内訳.push({ 名: x.名, 原価: c, 売価: p, 表: x.表 });
      });
      var 売価pool = (率0 ? 原価pool / 率0 : 原価pool * 1.5) * (1 + 上げ / 100);   // 昨年の原価率を保つ＋目安の上げ
      var 売価n = 売価表 + 売価pool; var 最低で = false;
    ...
    var 内訳文 = function (r) { return r.内訳.map(function (x) { return x.名 + ' ' + 円整(x.原価) + (x.売価 != null ? '→' + 円整(x.売価) : ''); }).join('／') + (r.売価pool ? '　（掛け率の無い項目の売価 ' + 円整(r.売価pool) + '）' : ''); };
    var h = '<div><span class="k">種:</span> <b>' + esc(ガイド種.番号) + '</b> ' + esc(f['製品名'] || '') + '　起票 ' + esc(日付(f['起票日'])) + '（' + 経過 + ' 年前 → 目安 +' + u + '%）'
      ...
      + '　※' + (表の項目.length ? '原価テーブルの行を使った項目: <b>' + esc(表の項目.join('・')) + '</b>。ほかは' : '原価テーブルに行が無いので、') + '版代は固定、印刷代は ' + Math.round(pf * 100) + '% 固定、ほかは数量比例で昨年の原価率を保ちます' + (最低受注 ? '。最低受注金額 ¥' + 円整(最低受注) : '') + '。段をクリックで数量と単価をフォームへ</div>'
      ...
      + rows.map(function (r) { return '<tr class="pick' + (r.n === qNow ? ' now' : '') + '" data-n="' + r.n + '" data-u="' + r.単価 + '" title="' + esc(内訳文(r)) + '"><td>' + 円整(r.n) + (r.n === qNow ? ' ◀ 今回' : '') + '</td><td>¥' + 円整(r.原価) + '</td><td>¥' + 円整(r.売価) + (r.最低で ? ' <span style="background:#fef3c7;color:#92400e;font-size:10px;padding:0 4px;border-radius:3px" title="最低受注金額で止めています">最低</span>' : '') + '</td><td>' + r.単価.toFixed(1) + '</td><td>' + r.粗利.toFixed(1) + '%</td><td>' + (r.前年比 >= 0 ? '+' : '') + r.前年比.toFixed(1) + '%</td>'
          + (内訳出す ? r.内訳.map(function (x) { return '<td style="font-size:11px;white-space:nowrap">' + 円整(x.原価) + '→' + (x.売価 != null ? 円整(x.売価) : '<span style="color:#94a3b8">率</span>') + '</td>'; }).join('') : '') + '</tr>'; }).join('')
      + '</tbody></table>';
  ...
  // KAKAKU-2: 原価テーブル（Hub マスタ）。取れたら描き直す。最低受注金額は PC に覚える
  var 原価表キャッシュ = null;
  function 原価表() { if (原価表キャッシュ) return 原価表キャッシュ; try { var c = JSON.parse(localStorage.getItem('fm_hubmaster_原価テーブル') || 'null'); if (c && c.行) return c.行; } catch (e) {} return []; }
  function 原価表を用意() { if (typeof Hubマスタを用意 !== 'function') return; Hubマスタを用意('原価テーブル').then(function (rows) { 原価表キャッシュ = rows || []; if (ガイド種) try { ガイドを描く(); } catch (e) {} }); }
  setTimeout(原価表を用意, 2500);
  try { var pm = localStorage.getItem('pg_min'); if (pm !== null && $('pg-min')) $('pg-min').value = pm; } catch (e) {}
  if ($('pg-min')) $('pg-min').addEventListener('change', function () { try { localStorage.setItem('pg_min', $('pg-min').value.trim()); } catch (e) {} if (ガイド種) ガイドを描く(); });
  ...
  var 列 = [
    ...
    { c:'製品名',     h:'製品名' },
    { c:'品種',       h:'品種' },
    { c:'ロット契約単位', h:'数量', num:true, alt:'合計数1' },   // LOT-1: 数量＝ロット契約単位（Hub の「数量」）。無ければ 合計数1
  ...
  function 一覧を出す(r) {
    ...
    function 行の札(row) {
      ...
      列.forEach(function (k) {
        var v = row[k.c]; if ((v == null || v === '') && k.alt) v = row[k.alt];
  ...
  function 索引を用意() {
    ...
    return 索引を開くDB().then(function (db) {
      return new Promise(function (res) {
        ...
        var 古い = !o || !o.取った || (Date.now() - new Date(o.取った).getTime()) > 有効 || (o.列 && o.列.indexOf('ロット契約単位') < 0);   // 列が増えたら取り直す（LOT-1）
  ...
  function 手元で探す(開いてよい, フォーム条件) {
    ...
    for (var i = 0; i < 行.length; i++) {
      ...
      if (!段階 && !段階が合う(r[P['案件区分']])) continue;   // STAGE-1: 段階を選んでいなければ、この画面の段階だけ
  ...
  function 段階の注意(f) {
    ...
    状態('この伝票は「' + (k || '受注') + '」です。' + (モード === 'mitsu' ? '受注入力' : '見積入力') + 'で扱う伝票なので、ここでは参照と Repeat だけにしてください', 'warn');
    try { var u = location.pathname + '?mode=' + 先 + '&no=' + encodeURIComponent(no); if (!$('fm-stage-jump')) { var a = document.createElement('a'); a.id = 'fm-stage-jump'; a.style.cssText = 'margin-left:8px;color:#fff;text-decoration:underline'; stat.appendChild(a); } $('fm-stage-jump').href = u; $('fm-stage-jump').textContent = (先 === 'mitsu' ? '見積入力' : '受注入力') + 'で開く'; } catch (e) {}
  }
  // 同じ案件の伝票（索引から）。案件ID が同じもの ＋ 前回伝票番号でつながるもの（前後 10 段まで・全年）。新しい順
  function 案件の伝票() {
    var out = [], seen = {}; if (!現在) return out;
    var me = String(現在.fields['伝票番号'] || 現在.fields['見積番号'] || ''); var 案件 = String(現在.fields['案件ID'] || '');
    out.push({ no: me, recordId: 現在.recordId, 区分: String(現在.fields['案件区分'] || '受注'), 起票日: String(現在.fields['起票日'] || ''), 数量: 現在.fields['ロット契約単位'] || 現在.fields['合計数1'] || '', 売価: 現在.fields['売価金額'] || '', 案件ID: 案件, 前: String(現在.fields['前回伝票番号'] || ''), 今: true }); seen[me] = 1;
    if (!索引) return out;
    var P = 索引位置; var by番号 = {}, by前 = {}, by案件 = {};
    索引.行.forEach(function (r) { var no = String(r[P['伝票番号']] || r[P['見積番号']] || '').trim(); if (!no) return; var o = { no: no, recordId: String(r[P['recordId']] || ''), 区分: String(r[P['案件区分']] || '受注'), 起票日: String(r[P['起票日']] || ''), 数量: r[P['ロット契約単位']] || r[P['合計数1']] || '', 売価: r[P['売価金額']] || '', 案件ID: String(r[P['案件ID']] || ''), 前: String(r[P['前回伝票番号']] || '') };
      by番号[no] = o; if (o.前) (by前[o.前] = by前[o.前] || []).push(o); if (o.案件ID) (by案件[o.案件ID] = by案件[o.案件ID] || []).push(o); });
    var queue = [out[0]], hops = 0;
    while (queue.length && hops < 400) { var x = queue.shift(); hops++;
      var near = [].concat(x.案件ID ? (by案件[x.案件ID] || []) : [], x.前 && by番号[x.前] ? [by番号[x.前]] : [], by前[x.no] || []);
      near.forEach(function (o) { if (seen[o.no]) return; seen[o.no] = 1; out.push(o); queue.push(o); }); }
    var 日 = function (d) { var m = /^(\d{1,2})\/(\d{1,2})\/(\d{4})$/.exec(d || ''); return m ? m[3] + ('0' + m[1]).slice(-2) + ('0' + m[2]).slice(-2) : String(d || '').replace(/-/g, ''); };
    out.sort(function (a, b) { return 日(b.起票日).localeCompare(日(a.起票日)) || b.no.localeCompare(a.no); });
    return out;
  }
  function リピートを選ぶ(種別) {
    if (!現在) { alert('先に元にする伝票を読み込んでください'); return; }
    種別 = 種別 || (モード === 'mitsu' ? '' : '受注');
    if (!索引) { 状態('索引を待っています…', ''); var w0 = 0; var t = setInterval(function () { w0++; if (索引 || w0 > 20) { clearInterval(t); リピートを選ぶ(種別); } }, 500); return; }
    var list = 案件の伝票(); var old = $('fm-repeat-pick'); if (old) old.remove();
    var m = document.createElement('div'); m.id = 'fm-repeat-pick'; m.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,.45);z-index:9999;display:flex;align-items:center;justify-content:center';
    var 種別欄 = モード === 'mitsu'
      ? '<div style="margin:6px 0">起こす段階: <label><input type="radio" name="rp-k" value="予算見積"' + (種別 === '予算見積' || !種別 ? ' checked' : '') + '> 予算見積</label>　<label><input type="radio" name="rp-k" value="見積"' + (種別 === '見積' ? ' checked' : '') + '> 見積</label></div>'
      : '<div style="margin:6px 0">起こす段階: <b>受注</b>（前回伝票番号に元の番号が入ります）</div>';
    m.innerHTML = '<div style="background:#fff;border-radius:8px;padding:16px 18px;width:min(720px,96vw);max-height:90vh;overflow:auto;font-size:13px;box-shadow:0 10px 40px rgba(0,0,0,.3)">'
      + '<div style="font-size:15px;font-weight:700">Repeat 登録 — 元にする伝票を選んでください</div>'
      + '<div style="color:#475569;margin:4px 0">同じ案件の伝票を新しい順に出しています（予算見積・見積・受注、全年）。一番新しいものを選んであります。昨年受注が無ければ予算見積や見積から起こせます</div>'
      + 種別欄
    ...
    $('rp-go').onclick = function () {
      var i = Number((m.querySelector('input[name="rp-src"]:checked') || {}).value || 0); var src = list[i]; if (!src) return;
  ...
  (function () {
    ...
    if (モード === 'juchu' || モード === '') { k.classList.add('fm-taihi'); var lb = k.previousElementSibling; if (lb && lb.classList.contains('lb')) lb.classList.add('fm-taihi'); return; }
    if (モード === 'mitsu') {
      Array.prototype.forEach.call(k.options, function (o) { if (o.value === '受注') { o.textContent = '受注（受注処理する）'; } });
  ...
  (function () {
    ...
    var b6 = document.createElement('button'); b6.className = 'fb'; b6.textContent = 'マスタ管理'; b6.title = '用紙・封筒・得意先・ユーザー名・基本原価のマスタ。FileMaker のマスタは Hub で直して反映';
    b6.onclick = function () { window.open('master.html', '_blank'); }; lay.parentNode.insertBefore(b6, lay);
    // COPY-1: 仕様を文章にしてコピー（印刷手配の担当に LINE WORKS などで渡す）
  ...
  // ================================================================ HIDE-1: 使っていない項目を退避する
  var 画面項目 = {};          // id → { label, 状態, 退避日, 戻し日, 削除日, 入力数, 変更数, 最終入力, 記録開始 }
  var 退避モード = false, 使用ログ = {}, 使用済 = {};
  function 項目の鍵(el) { if (!el) return ''; if (el.id) return el.id; var t = (el.textContent || '').trim().slice(0, 30); return t ? (el.tagName.toLowerCase() + ':' + t) : ''; }
  function 項目の名前(el) {
    if (!el) return ''; var p = el.previousElementSibling; if (p && p.classList && p.classList.contains('lb')) return p.textContent.trim().slice(0, 40);
    var lab = el.closest('label'); if (lab) return lab.textContent.trim().slice(0, 40);
    var td = el.closest('td'); if (td) { var tr = td.closest('tr'), tb = tr && tr.closest('table'); if (tb) { var i = Array.prototype.indexOf.call(tr.children, td); var th = tb.querySelector('th:nth-child(' + (i + 1) + ')'); if (th) return th.textContent.trim().slice(0, 40); } }
    return ((el.textContent || '').trim() || el.placeholder || el.title || el.id || '').slice(0, 40);
  }
  function 鍵の要素(id) { if (id.indexOf(':') < 0) return $(id); var m = /^(\w+):([\s\S]*)$/.exec(id); if (!m) return null; var c = document.querySelectorAll(m[1]); for (var i = 0; i < c.length; i++) { if ((c[i].textContent || '').trim().slice(0, 30) === m[2]) return c[i]; } return null; }
  function 退避できる(target) { if (!target || !target.closest) return null; var t = target.closest('input,select,textarea,button,.fb'); if (!t) return null; if (t.closest('#fm-shelf') || t.id === 'fm-taihi' || t.closest('table.g tbody') || t.closest('#fmfind')) return null; return t; }
  function 隠す(el, on) { if (!el) return; el.classList.toggle('fm-taihi', on); var p = el.previousElementSibling; if (p && p.classList && p.classList.contains('lb')) p.classList.toggle('fm-taihi', on); var lab = el.closest('label'); if (lab) lab.classList.toggle('fm-taihi', on); }
  function 退避を適用() { Object.keys(画面項目).forEach(function (id) { var r = 画面項目[id]; 隠す(鍵の要素(id), r.状態 === '退避' || r.状態 === '削除'); }); }
  function 画面項目を読む(強制) {
    try { var c = JSON.parse(localStorage.getItem('fm_gamen_koumoku') || 'null'); if (c && c.行 && !強制 && Date.now() - new Date(c.取った).getTime() < 10 * 60 * 1000) { 画面項目 = {}; c.行.forEach(function (r) { 画面項目[r.id] = r; }); 退避を適用(); return Promise.resolve(); } } catch (e) {}
    if (!(window.FM名乗っている ? FM名乗っている() : false)) return Promise.resolve();
    return 呼ぶ('Hubマスタ_一覧', ['画面項目']).then(function (j) { 画面項目 = {}; (j.行 || []).forEach(function (r) { 画面項目[r.id] = r; }); try { localStorage.setItem('fm_gamen_koumoku', JSON.stringify({ 取った: new Date().toISOString(), 行: j.行 || [] })); } catch (e) {} 退避を適用(); }).catch(function () {});
  }
  function 状態を変える(id, label, st) {
    var r = 画面項目[id] = 画面項目[id] || { id: id, label: label }; r.状態 = st; var d = new Date().toISOString(); if (st === '退避') r.退避日 = d; else if (st === '表示') r.戻し日 = d; else r.削除日 = d;
    隠す(鍵の要素(id), st !== '表示'); 棚を描く(); try { localStorage.removeItem('fm_gamen_koumoku'); } catch (e) {}
    呼ぶ('画面項目_退避', [id, label, st]).then(function (j) { if (j && j.行) 画面項目[id] = j.行; 棚を描く(); }).catch(function (e) { 状態('退避を記録できません: ' + String(e.message || e), 'ng'); });
  }
  document.addEventListener('click', function (e) {
    if (!退避モード) return; var t = 退避できる(e.target); if (!t) return; e.preventDefault(); e.stopPropagation();
    var id = 項目の鍵(t); if (!id) return; var label = 項目の名前(t);
    if (!confirm('「' + label + '」を退避します（消えます）。退避棚からいつでも戻せます。よいですか？')) return;
    状態を変える(id, label, '退避');
  }, true);
  // 使われ方: 人が打ったものだけ（isTrusted）。読込時に空だった欄＝新規入力、入っていた欄＝変更。1 伝票につき 1 回ずつ数える
  document.addEventListener('input', function (e) {
    if (!e.isTrusted) return; var el = e.target; if (!el || !el.id || el.closest('#fm-shelf') || el.closest('#fmfind') || el.closest('#fmbar')) return;
    var rec = 現在 ? String(現在.recordId || 現在.fields['伝票番号'] || '') : 'new'; var key = el.id + '|' + rec; if (使用済[key]) return; 使用済[key] = 1;
    var was = String(読込時[el.id] || '').trim(); var g = 使用ログ[el.id] = 使用ログ[el.id] || { id: el.id, label: 項目の名前(el), 入力: 0, 変更: 0 }; if (was) g.変更++; else g.入力++;
  }, true);
  function 使用ログを送る() {
    var rows = Object.keys(使用ログ).map(function (k) { return 使用ログ[k]; }); if (!rows.length) return; 使用ログ = {};
    if (!(window.FM名乗っている ? FM名乗っている() : false)) return;
    呼ぶ('画面項目_使った', [rows]).catch(function () { rows.forEach(function (r) { var g = 使用ログ[r.id] = 使用ログ[r.id] || { id: r.id, label: r.label, 入力: 0, 変更: 0 }; g.入力 += r.入力; g.変更 += r.変更; }); });
  }
  setInterval(使用ログを送る, 90 * 1000); window.addEventListener('beforeunload', 使用ログを送る);
  function 日数(iso) { if (!iso) return null; var t = new Date(iso).getTime(); if (isNaN(t)) return null; return Math.floor((Date.now() - t) / 86400000); }
  function 日短(iso) { return iso ? String(iso).slice(0, 10) : '—'; }
  function 棚を開閉() { var sh = $('fm-shelf'); if (!sh) { sh = document.createElement('div'); sh.id = 'fm-shelf'; document.body.appendChild(sh); } if (sh.style.display === 'block') { sh.style.display = 'none'; 退避モードにする(false); return; } sh.style.display = 'block'; 画面項目を読む(true).then(棚を描く); 棚を描く(); }
  function 退避モードにする(on) { 退避モード = !!on; document.body.classList.toggle('taihi-mode', 退避モード); var b = $('sh-mode'); if (b) { b.textContent = 退避モード ? '退避モードをやめる' : '退避モードにする（クリックで退避）'; b.style.background = 退避モード ? '#b91c1c' : ''; b.style.color = 退避モード ? '#fff' : ''; } }
  function 棚を描く() {
    var sh = $('fm-shelf'); if (!sh || sh.style.display !== 'block') return;
    var rows = Object.keys(画面項目).map(function (k) { return 画面項目[k]; });
    var 退避中 = rows.filter(function (r) { return r.状態 === '退避'; }).sort(function (a, b) { return String(a.退避日 || '').localeCompare(String(b.退避日 || '')); });
    var 削除済 = rows.filter(function (r) { return r.状態 === '削除'; });
    var 候補 = rows.filter(function (r) { return r.状態 !== '退避' && r.状態 !== '削除' && 鍵の要素(r.id) && (日数(r.最終入力) === null ? 日数(r.記録開始) >= 90 : 日数(r.最終入力) >= 90); }).sort(function (a, b) { return String(a.最終入力 || '').localeCompare(String(b.最終入力 || '')); });
    var 使い = function (r) { return '入力 ' + (r.入力数 || 0) + '・変更 ' + (r.変更数 || 0) + '<br>最終入力 ' + 日短(r.最終入力) + (r.記録開始 ? '<br><span style="color:#94a3b8">記録 ' + 日短(r.記録開始) + '〜</span>' : ''); };
    var h = '<div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap"><b style="color:#b91c1c;font-size:13px">🗄 退避棚</b><button id="sh-mode" type="button">退避モードにする（クリックで退避）</button><button id="sh-close" type="button" style="margin-left:auto">閉じる</button></div>'
      + '<div style="color:#64748b;margin:4px 0 6px">退避モードで入力欄やボタンをクリックすると消えます（ラベルも）。「戻す」で同じ場所に戻ります。3 か月戻さなかったものは「削除」で外してよい目安です（削除しても戻せます）。入力の数は人が打ったものだけで、コピーで最初から入っていた値は数えません</div>'
      + '<b>退避中 ' + 退避中.length + '</b><table><thead><tr><th>項目</th><th>退避した日</th><th>経過</th><th>使われ方</th><th></th></tr></thead><tbody>'
      + (退避中.length ? 退避中.map(function (r) { var d = 日数(r.退避日); return '<tr><td>' + esc(r.label || r.id) + '<br><span style="color:#94a3b8">' + esc(r.id) + '</span></td><td>' + 日短(r.退避日) + (r.戻し日 ? '<br><span style="color:#94a3b8">前に戻した ' + 日短(r.戻し日) + '</span>' : '') + '</td><td class="' + (d >= 90 ? 'old' : '') + '">' + (d === null ? '—' : d + ' 日') + (d >= 90 ? '<br>3 か月経過 → 削除してよい' : '') + '</td><td>' + 使い(r) + '</td><td style="white-space:nowrap"><button data-act="表示" data-id="' + esc(r.id) + '">戻す</button> <button data-act="削除" data-id="' + esc(r.id) + '" style="color:#b91c1c">削除</button></td></tr>'; }).join('') : '<tr><td colspan="5" style="color:#94a3b8">まだありません</td></tr>') + '</tbody></table>'
      + '<b style="display:block;margin-top:8px">使っていない候補 ' + 候補.length + '</b><div style="color:#64748b">最終入力が 90 日以上前（または記録を始めて 90 日入力なし）の表示中の項目</div><table><thead><tr><th>項目</th><th>使われ方</th><th></th></tr></thead><tbody>'
      + (候補.length ? 候補.slice(0, 40).map(function (r) { return '<tr><td>' + esc(r.label || r.id) + '<br><span style="color:#94a3b8">' + esc(r.id) + '</span></td><td>' + 使い(r) + '</td><td><button data-act="退避" data-id="' + esc(r.id) + '">退避</button></td></tr>'; }).join('') : '<tr><td colspan="3" style="color:#94a3b8">まだありません（使われ方の記録が 90 日たまると出ます）</td></tr>') + '</tbody></table>'
      + (削除済.length ? '<details style="margin-top:8px"><summary>削除済み ' + 削除済.length + '</summary><table><tbody>' + 削除済.map(function (r) { return '<tr><td>' + esc(r.label || r.id) + '</td><td>削除 ' + 日短(r.削除日) + '</td><td><button data-act="表示" data-id="' + esc(r.id) + '">戻す</button></td></tr>'; }).join('') + '</tbody></table></details>' : '');
    sh.innerHTML = h;
    $('sh-close').onclick = function () { sh.style.display = 'none'; 退避モードにする(false); };
    $('sh-mode').onclick = function () { 退避モードにする(!退避モード); };
    if (退避モード) 退避モードにする(true);
    Array.prototype.forEach.call(sh.querySelectorAll('button[data-act]'), function (b) { b.onclick = function () { var id = b.getAttribute('data-id'), r = 画面項目[id] || {}; var act = b.getAttribute('data-act'); if (act === '削除' && !confirm('「' + (r.label || id) + '」を削除します（退避棚の「削除済み」から戻せます）。よいですか？')) return; 状態を変える(id, r.label || id, act); }; });
  }
  setTimeout(function () { 画面項目を読む(false); }, 2000);
  setInterval(function () { if (!Object.keys(画面項目).length) 画面項目を読む(false); }, 30000);   // サインインが後から済んだとき用
  function 仕様の文を作る() {
    var v = function (id) { var x = $(id); return x ? String(x.value || '').trim() : ''; };
    var 行 = [];
    var no = v('f-denpyo') || v('f-mitsuno') || (現在 ? String(現在.fields['伝票番号'] || 現在.fields['見積番号'] || '') : '');
    var 区分 = v('f-kubun') || (現在 ? String(現在.fields['案件区分'] || '') : '');
    行.push('【' + (区分 || '受注') + ' ' + no + '】');
    var 客 = v('f-cust'); var ユ = v('f-user'); 行.push('顧客: ' + 客 + (ユ ? '／' + ユ : ''));
    行.push('製品名: ' + v('f-item') + (v('f-prod') ? '（' + v('f-prod') + '）' : ''));
    var 数 = v('f-lotunit'), 単 = v('f-unit'), セ = v('f-lotset');   // LOT-1: 数量＝ロット数量（契約単位）
    行.push('数量: ' + (数 ? 数 + (単 || '') : '') + (セ && セ !== 数 ? '（セット数 ' + セ + '）' : ''));
    if (v('f-w') || v('f-h')) 行.push('サイズ: ' + v('f-w') + '×' + v('f-h') + 'mm' + (v('f-size') ? '（' + v('f-size') + '）' : ''));
    var 紙 = function (n) { return [v('p' + n + '-kind'), v('p' + n + '-mk'), v('p' + n + '-nm'), v('p' + n + '-th'), v('p' + n + '-sz')].filter(Boolean).join(' '); };
    var 色 = function (n) { var a = []; var i1 = v('k' + n + '-iro1'), i2 = v('k' + n + '-iro2'), ink = v('k' + n + '-ink'), ura = v('k' + n + '-ura'), han = v('k' + n + '-han');
      if (i1 || ink) a.push('表 ' + [i1 ? i1 + '色' : '', ink].filter(Boolean).join(' ')); if (i2 || ura) a.push('裏 ' + [i2 ? i2 + '色' : '', ura].filter(Boolean).join(' ')); if (han) a.push('版下 ' + han); return a.join('／'); };
    var 手 = function (種, n) { var x = ((手配 || {})[種] || {})[String(n)]; return x && x.k ? x.k + (x.who ? ' ' + x.who : '') : ''; };
    if (v('f-cover') === 'あり') { var c = 紙(0), cc = 色(0); 行.push('表紙: ' + (c || '（用紙未記入）') + (cc ? '　印刷 ' + cc : '') + (手('用紙', 0) ? '　[' + 手('用紙', 0) + ']' : '')); }
    else 行.push('表紙: なし');
    var n本 = parseInt(v('f-parts'), 10) || 1; if (n本 > 9) n本 = 9; var 本 = [];
    for (var i = 1; i <= 9; i++) { var p = 紙(i), k = 色(i); if (!p && !k) continue; if (i > n本 && !p) continue; 本.push((n本 > 1 ? 'パーツ' + i + ': ' : '') + (p || '（用紙未記入）') + (k ? '　印刷 ' + k : '') + (手('用紙', i) ? '　[' + 手('用紙', i) + ']' : '')); }
    if (本.length) { if (n本 > 1) { 行.push('本文の用紙・印刷色:'); 本.forEach(function (x) { 行.push('　' + x); }); } else 行.push('本文の用紙・印刷色: ' + 本[0]); }
    else 行.push('本文の用紙・印刷色: （未記入）');
    if (v('f-bind')) 行.push('加工: ' + v('f-bind'));
    // 納期は最終納期。外注に出すときの納期は遅くても 1 日前（本多さん 9/25）。曜日も付けて判断しやすく
    var 曜 = ['日', '月', '火', '水', '木', '金', '土'];
  ...
  window.FM仕様文 = 仕様の文を作る;
  function 仕様をコピー() {
    if (!現在) { 状態('伝票を読み込んでからコピーしてください', 'ng'); return; }
    var t = 仕様の文を作る();
    var done = function () { 状態('仕様をコピーしました（LINE WORKS やメールに貼れます）', 'ok'); };
    var fallback = function () { var ta = document.createElement('textarea'); ta.value = t; ta.style.cssText = 'position:fixed;left:-9999px'; document.body.appendChild(ta); ta.select(); var ok = false; try { ok = document.execCommand('copy'); } catch (e) {} ta.remove(); if (ok) done(); else { window.prompt('コピーできませんでした。この文を選んでコピーしてください', t); } };
  ...
  function 列を整える() {
    ...
    Object.keys(先頭).forEach(function (t) {
      ...
      Array.prototype.forEach.call(tb.querySelectorAll('tr.r-cover'), function (tr) { tr.setAttribute('data-n', '0'); });   // COVER-1: 表紙行は 0
      Array.prototype.forEach.call(tb.querySelectorAll('tr.r-n, tr.r-cover'), function (tr) {
    ...
    if ($('f-cover')) $('f-cover').addEventListener('change', 手配が変わった);   // COVER-1: 表紙の有無は手配に覚える
  ...
  function 手配を集める() {
    ...
    if ($('f-cover') && 現在) o.表紙 = $('f-cover').value;   // COVER-1
    return o;
  }
  function 手配を置く(o) {
    o = o || {}; 手配読込中 = true;
  ...
  // YOSAN-1: 予算見積 ＝ 積み上げ × (1 + 上乗せ%)。上乗せは PC に覚える（案件ごとの上書きは事前情報に）
  var 積み上げ売価 = 0;
  function 予算上乗せ() { var jz = (手配 && 手配.事前) || {}; if (jz.予算上乗せ !== undefined && jz.予算上乗せ !== '') return Number(jz.予算上乗せ); var v = localStorage.getItem('yosan_markup'); return v === null ? 10 : Number(v); }
  function 予算行を出す() {
    var box = $('kn-yosan'); if (!box || !現在) return; var 区分 = String(現在.fields['案件区分'] || ''); var pct = 予算上乗せ();
    var 売今 = 金額(($('f-amt') || {}).value); var 数 = 金額(($('p1-tot') || {}).value) || 金額(($('f-lotunit') || {}).value) || 金額(現在.fields['合計数1']);
    // この案件で「予算見積と見積が同額」だった年があるか（索引から）
    var 同額 = [], id = String(現在.fields['案件ID'] || '');
    if (索引 && id) { var g = 案件の結果表()[id]; var P = 索引位置; if (g) { var by = {}; g.行.forEach(function (i) { var r = 索引.行[i]; var y = String(r[P['起票日']] || '').slice(6, 10); var kb = String(r[P['案件区分']] || ''); if (kb !== '予算見積' && kb !== '見積') return; (by[y] = by[y] || {})[kb] = 金額(r[P['売価金額']]); }); Object.keys(by).forEach(function (y) { if (by[y]['予算見積'] && by[y]['予算見積'] === by[y]['見積']) 同額.push(y); }); } }
    var yosan = 積み上げ売価 ? Math.round(積み上げ売価 * (1 + pct / 100)) : 0;
    var h = '<b style="color:#1e40af">予算見積</b>　上乗せ <input id="kn-markup" type="number" step="0.5" style="width:56px;text-align:right" value="' + pct + '">%'
      + (積み上げ売価 ? '　積み上げ ' + 円表示(積み上げ売価) + ' → 予算見積額 <b>' + 円表示(yosan) + '</b>' + (数 ? '（単価 ' + (Math.round(yosan / 数 * 100) / 100) + '）' : '') + (区分 === '予算見積' ? ' <button type="button" class="plain up" id="kn-yosan-set" style="font-size:11px;padding:1px 8px">売価に入れる</button>' : '') : '　積み上げ売価が出ると、ここに予算見積額が出ます')
      + (売今 && 積み上げ売価 ? '　いまの売価は積み上げ比 ' + (売今 >= 積み上げ売価 ? '+' : '') + (Math.round((売今 / 積み上げ売価 - 1) * 1000) / 10) + '%' : '')
      + (同額.length ? '<div style="color:#1e3a8a;margin-top:2px">この案件は ' + esc(同額.join('・')) + ' 年に予算見積と見積が同額でした → 見積依頼が来たら、予算見積をそのまま見積にできます（都度確認）。' + (区分 === '予算見積' ? '見積を起こすときは金額を変えずに保存すれば同じ流れです' : '') + '</div>' : (区分 === '予算見積' ? '<div style="color:#475569;margin-top:2px">見積依頼が来たら「見積」を押して起こし、予算見積の額のまま通せるか確認してください</div>' : ''));
    box.style.display = 'block'; box.innerHTML = h;
    var mi = $('kn-markup'); if (mi) mi.addEventListener('change', function () { var v = Number(mi.value); if (isNaN(v)) return; try { localStorage.setItem('yosan_markup', String(v)); } catch (e) {} if (手配 && 現在) { 手配.事前 = 手配.事前 || {}; 手配.事前.予算上乗せ = v; } 予算行を出す(); });
    var sb = $('kn-yosan-set'); if (sb) sb.onclick = function () { if (!yosan) return; if ($('f-amt')) { $('f-amt').value = yosan; $('f-amt').dispatchEvent(new Event('input', { bubbles: true })); } if ($('f-price') && 数) { $('f-price').value = Math.round(yosan / 数 * 100) / 100; $('f-price').dispatchEvent(new Event('input', { bubbles: true })); } try { if (typeof calc === 'function') calc(); } catch (e) {} 状態('予算見積額 ' + 円表示(yosan) + ' を売価に入れました（保存で FileMaker へ）', 'ok'); };
  }
  function 相場を出す() {
    var box = $('kn-souba'); if (!box) return; if (!現在) { box.style.display = 'none'; if ($('kn-yosan')) $('kn-yosan').style.display = 'none'; return; }
    積み上げ売価 = 0;
    // MASTER-1: 基本原価表（Hubマスタ）に同じ鍵があればそれを優先
    var f0 = 現在.fields; var myQ0 = 金額(($('p1-tot') || {}).value) || 金額(($('f-lotunit') || {}).value) || 金額(f0['合計数1']);
    var g0 = function (c) { var id = { '品種': 'f-hinshu2', 'サイズ横': 'f-w', 'サイズ縦': 'f-h', '色数1': 'k1-iro1', '色MAX1': 'k1-iro2', 'パーツ数': 'f-parts' }[c]; var v = id && $(id) ? $(id).value : f0[c]; if (c === '合計数1') return myQ0; return v == null ? '' : String(v).trim(); };
    var key0 = ['品種' + (g0('品種') || '—'), (g0('サイズ横') || '') + '×' + (g0('サイズ縦') || ''), (g0('色数1') || '?') + (g0('色MAX1') ? '/' + g0('色MAX1') : '') + '色', (g0('パーツ数') || '1') + 'P', 数量帯(金額(g0('合計数1')))].join(' | ');
    Hubマスタを用意('基本原価').then(function (rows) {
      var hit = rows.filter(function (r) { return String(r['鍵'] || '') === key0; })[0];
      if (hit) { var mk = 1.15; var now = function (k) { var id = { '用紙代': 'c-paper', '版代': 'c-plate', '梱包代': 'c-pack', '配送代': 'c-ship', '印刷代': 'c-print', '加工賃': 'c-work', '人件費': 'c-labor', '合計金額': 'c-total' }[k]; return 金額(($(id) || {}).value); };
        var B = 材料費(null, now); var 材料売今 = (B.用紙 + B.外注) * mk + now('版代') + now('梱包代') + now('配送代'); var 作業今 = myQ0 ? 金額(hit['作業原価_1000']) * myQ0 / 1000 : 0; var 率 = 金額(hit['掛け率']) || 1; var 積み上げ = 材料売今 + 作業今 * 率; var 売今 = 金額(($('f-amt') || {}).value);
        積み上げ売価 = 積み上げ; 予算行を出す();
        box.style.display = 'block'; box.innerHTML = '<b style="color:#3730a3">基本原価表</b>（' + esc(key0) + '、' + esc(hit['件数'] || '') + ' 件から）　作業原価 ' + 円表示(金額(hit['作業原価_1000'])) + '/1,000　掛け率 ' + 率 + '×' + (hit['加工内訳'] ? '　' + esc(hit['加工内訳']) : '') + (myQ0 ? '　→ 積み上げ売価 <b>' + 円表示(積み上げ) + '</b>（材料 ' + 円表示(材料売今) + ' ＋ 作業 ' + 円表示(作業今) + ' × ' + 率 + '）' + (売今 ? '　いま ' + 円表示(売今) + ' <span style="color:' + (売今 >= 積み上げ ? '#166534' : '#b91c1c') + '">' + (売今 >= 積み上げ ? '+' : '') + Math.round(売今 - 積み上げ).toLocaleString() + '</span>' : '') : '') + '　<a href="master.html?t=基本原価" target="_blank" style="color:#3730a3">表</a>'; return; }
      相場を出す2();
    });
  }
  function 相場を出す2() {
    ...
    単価データを用意().then(function (j) {
      ...
      for (var lvl = 0; lvl <= 3; lvl++) {
        ...
        if (hit.length >= 3 || lvl === 3) {
          if (!hit.length) { box.style.display = 'block'; box.innerHTML = '<span style="color:#475569">この仕様の相場: 似た受注が見つかりません（<a href="tanka-bunseki.html" target="_blank">単価分析</a>）</span>'; 予算行を出す(); return; }
  ...
  // MASTER-1: Hub のマスタ（ユーザー名・封筒）を候補に。1 日 1 回取る
  var Hubマスタ = {};
  function Hubマスタを用意(名) {
    if (Hubマスタ[名]) return Promise.resolve(Hubマスタ[名]);
    try { var c = JSON.parse(localStorage.getItem('fm_hubmaster_' + 名) || 'null'); if (c && c.取った && Date.now() - new Date(c.取った).getTime() < 24 * 3600 * 1000) { Hubマスタ[名] = c.行; return Promise.resolve(c.行); } } catch (e) {}
    if (!(window.FM名乗っている ? FM名乗っている() : false)) return Promise.resolve([]);
    return 呼ぶ('Hubマスタ_一覧', [名]).then(function (j) { Hubマスタ[名] = j.行 || []; try { localStorage.setItem('fm_hubmaster_' + 名, JSON.stringify({ 取った: new Date().toISOString(), 行: j.行 || [] })); } catch (e) {} return Hubマスタ[名]; }).catch(function () { return []; });
  }
  function ユーザー名の候補() {
    var u = $('f-user'); if (!u) return; var dl = $('dl-user-hub'); if (!dl) { dl = document.createElement('datalist'); dl.id = 'dl-user-hub'; document.body.appendChild(dl); u.setAttribute('list', 'dl-user-hub'); }
    var code = String(($('f-custcd') || {}).value || '').trim();
    Hubマスタを用意('ユーザー名').then(function (rows) { dl.innerHTML = rows.filter(function (r) { return !code || String(r['得意先コード'] || '') === code; }).map(function (r) { return '<option value="' + esc(r['ユーザー名'] || '') + '">' + esc(r['得意先名'] || '') + '</option>'; }).join(''); });
  }
  function 封筒の候補() {
    Hubマスタを用意('封筒').then(function (rows) { if (!rows.length) return; var dl = $('dl-fuuto'); if (!dl) { dl = document.createElement('datalist'); dl.id = 'dl-fuuto'; document.body.appendChild(dl); }
      dl.innerHTML = rows.map(function (r) { var nm = [r['型'], r['メーカー'], r['品名'], r['窓'] ? '窓' + r['窓'] : '', r['色']].filter(Boolean).join(' '); return '<option value="' + esc(nm) + '">仕入 ' + esc(r['仕入単価'] || '') + '</option>'; }).join('');
      for (var i = 1; i <= 9; i++) { var el = $('p' + i + '-nm'); if (el && !el.getAttribute('list')) { el.setAttribute('list', 'dl-fuuto'); el.addEventListener('change', (function (n) { return function () { var v = this.value.trim(); var hit = rows.filter(function (r) { return [r['型'], r['メーカー'], r['品名'], r['窓'] ? '窓' + r['窓'] : '', r['色']].filter(Boolean).join(' ') === v; })[0]; if (hit && hit['仕入単価'] && $('p' + n + '-up') && !$('p' + n + '-up').value) { $('p' + n + '-up').value = hit['仕入単価']; $('p' + n + '-up').dispatchEvent(new Event('input', { bubbles: true })); } }; })(i)); } } });
  }
  setTimeout(function () { ユーザー名の候補(); 封筒の候補(); }, 2500); window.addEventListener('fm-signin', function () { setTimeout(function () { ユーザー名の候補(); 封筒の候補(); }, 2000); });
  if ($('f-custcd')) $('f-custcd').addEventListener('change', ユーザー名の候補);
  ...
  // STAGE-1: 受注入力は受注（と区分の無い従来の受注）だけ、見積入力は予算見積・見積・失注だけ
  function 段階が合う(区分) { 区分 = String(区分 || ''); if (モード === 'ichiran') return true; if (モード === 'mitsu') return 区分 === '予算見積' || 区分 === '見積' || 区分 === '失注'; return 区分 === '' || 区分 === '受注'; }
  function 送り一覧() {
    if (!索引) return [];
    if (送り && 送り元 === 索引.行) return 送り;
    var c = 索引位置['伝票番号'], d = 索引位置['起票日'], kb = 索引位置['案件区分']; var rows = [];
    索引.行.forEach(function (r, i) { var no = String(r[c] || (モード === 'mitsu' ? r[索引位置['見積番号']] : '') || '').trim(); if (!no) return; if (kb != null && !段階が合う(r[kb])) return; rows.push([索引日 ? 索引日[i] : 0, no]); });
